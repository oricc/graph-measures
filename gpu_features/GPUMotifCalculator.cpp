...
GPUMotifCalculator::GPUMotifCalculator(int level, bool directed) : directed(directed), nodeVariations(NULL), allMotifs(NULL), removalIndex(
																																  NULL),
																   sortedNodesByDegree(NULL), fullGraph(false)
{
	//check level
	if (level != 3 && level != 4)
...
void GPUMotifCalculator::CopyAllToDevice()
{
	...
	// Feature matrix
	unsigned int size = this->mGraph->GetNumberOfNodes() * this->nodeVariations->Size() * sizeof(unsigned int);
	cudaMallocManged(this->deviceFeatures,size);

	// Original graph
	cudaMallocManged(&deviceOriginalGraphOffsets,(this->mGraph->GetNumberOfNodes() + 1)*sizeof(int64));
	cudaMallocManged(&deviceOriginalGraphNeighbors,(this->mGraph->GetNumberOfEdges())*sizeof(unsigned int));
	std::memcpy(deviceOriginalGraphOffsets,this->mGraph->GetOffsetList(),(this->mGraph->GetNumberOfNodes() + 1)*sizeof(int64));
	std::memcpy(deviceOriginalGraphNeighbors,this->mGraph->GetNeighborList(),(this->mGraph->GetNumberOfEdges())*sizeof(unsigned int));

	// Full graph
	cudaMallocManged(&deviceFullGraphOffsets,(this->fullGraph.GetNumberOfNodes() + 1)*sizeof(int64));
	cudaMallocManged(&deviceFullGraphNeighbors,(this->fullGraph.GetNumberOfEdges())*sizeof(unsigned int));
	std::memcpy(deviceOriginalGraphOffsets,this->fullGraph.GetOffsetList(),(this->fullGraph.GetNumberOfNodes() + 1)*sizeof(int64));
	std::memcpy(deviceOriginalGraphNeighbors,this->fullGraph.GetNeighborList(),(this->fullGraph.GetNumberOfEdges())*sizeof(unsigned int));
...
vector<vector<unsigned int> *> *GPUMotifCalculator::Calculate()
{
	...
	int blockSize = 256;
	int numBlocks = (this->mGraph->GetNumberOfNodes() + blockSize - 1) / blockSize;
...
void GPUMotifCalculator::Motif3Subtree(unsigned int root)
{
	...
	for (int64 i = offsets[root]; i < offsets[root + 1]; i++)
	{
		for (int64 j = i + 1; j < offsets[root + 1]; j++)
		{
			...
			if (this->deviceRemovalIndex[n1] <= idx_root || this->deviceRemovalIndex[n2] <= idx_root) // motif already handled
				continue;
			...
			if ((n1 < n2) && !(this->AreNeighbors(n1, n2) || this->AreNeighbors(n2, n1))) // check n1, n2 not neighbors
...
{
	int idx_root = this->deviceRemovalIndex[root];									// root_idx is also our current iteration -
	thrust::device_vector<char> visited_vertices(this->mGraph->GetNumberOfNodes()); // every node_idx smaller than root_idx is already handled
	...
	for (int64 i = offsets[root]; i < end; i++)
	{
		for (int64 j = i + 1; j < end; j++)
		{
			...
			for (int64 k = j + 1; k < end; k++)
			{
				...
				unsigned int n13 = neigbors[k];
				if (this->deviceRemovalIndex[n11] <= idx_root || this->deviceRemovalIndex[n12] <= idx_root || this->deviceRemovalIndex[n13] <= idx_root) // motif already handled
					continue;
	...
	{ // loop first neighbors
		...
		{ // loop second neighbors
			...
			if (this->deviceRemovalIndex[n2] <= idx_root) // n2 already handled
				continue;
			if (visited_vertices.find(n2) == visited_vertices.end())
			{ // check if n2 was *not* visited
		...
		for (int64 i = offsets[n1]; i < end; i++)
		{
			for (int64 j = i + 1; j < end; j++)
			{
				...
				if (this->deviceRemovalIndex[n21] <= idx_root || this->deviceRemovalIndex[n22] <= idx_root) // motif already handled
					continue;
		...
		for (int64 n2_idx = offsets[n1]; n2_idx < offsets[n1 + 1]; n2_idx++)
		{ // loop second neighbors (third time's the charm)
			unsigned int n2 = neighbors[n2_idx];
			if (this->deviceRemovalIndex[n2] <= idx_root) // n2 already handled
				continue;
			...
			{ // loop third neighbors
				...
				if (this->deviceRemovalIndex[n3] <= idx_root) // n2 already handled
					continue;
				if (visited_vertices.find(n3) == visited_vertices.end())
				{ // check if n3 was *not* visited
					visited_vertices[n3] = 3;
					if (visited_vertices[n2] == 2)
					{											   // check if n2 is a visited second neighbor
...
bool GPUMotifCalculator::AreNeighbors(unsigned int p, unsigned int q)
{
	...
	thrust::device_ptr<unsigned int> neighbor_ptr(this->deviceOriginalGraphNeighbors);
	int64 begin_offset = deviceOriginalGraphOffsets[p];
	int64 end_offset = deviceOriginalGraphOffsets[p + 1];

	thrust::device_vector<unsigned int> values(1);
	values[0] = q;
	thrust::device_vector<bool> output(1);
	thrust::binary_search(neighbor_ptr + begin_offset, neighbor_ptr + end_offset,
						  values.begin(), values.end(),
						  output.begin(),
						  thrust::less<unsigned int>());
...
void GPUMotifCalculator::GroupUpdater(unsigned int[] group, int size)
{
	...
	if (motifNumber != -1)
		for (int i = 0; i < size; i++)
			atomicAdd(deviceFeatures + (motifNumber + this->numOfMotifs * group[i]), 1); //atomic add + access as 1D array : features[motif + M*node]
...
int GPUMotifCalculator::GetGroupNumber(unsigned int[] group, int size)
{
	...
	GPUMotifCalculator::~GPUMotifCalculator()
	{
		...
		cudaFree(deviceOriginalGraphOffsets);
		cudaFree(deviceOriginalGraphNeighbors;

		// For the full graph
		cudaFree(deviceFullGraphOffsets);
		cudaFree(deviceFullGraphNeighbors);

		// Feature array
		cudaFree(deviceFeatures);
	}
